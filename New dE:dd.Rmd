...
starting_point = which(out3[ , 2] >= Emax[1])[1]
after_Emax1 = out3[ ,2][starting_point:nrow(out3)]
total_length = length(after_Emax1) + length(before_Emax1)
if (total_length > length(dose)) {
  after_Emax1 = after_Emax1[-((length(after_Emax1) - (total_length - length(dose))) + 1:length(after_Emax1))]
} else
    dose = dose[-((length(dose) - (length(dose) - total_length) + 1:length(after_Emax1)))]
